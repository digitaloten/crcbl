//! hud's one menu: the pause panel.
//!
//! ```text
//!   paused ──▶ Paused
//!   running ─▶ none
...
//! this sample declares no action of its own — [`crate::app::Hud`]'s `menu_action`
//! answers `None` for every id it is asked about, and its `MenuAction` type is
//! [`core::convert::Infallible`] because there is genuinely no value it could
//! ever be handed.
//!
//! That is not a menu waiting to be filled in. There is nothing for a hud button
//! to do: the page is driven end to end by the server's ticker and takes no
//! input, so a fourth item would have to invent a thing for this sample to be.
//! The panel exists because `ESC` pauses in every sample and a paused screen
...
pub enum MenuKind {
    /// The ticker is running: no menu at all.
    #[default]
    None,
    /// The loop has stopped advancing the ticker.
    Paused,
}
...
/// hud's menus, keyed by the state each belongs to.
