...
//!
//! `backdrop` (clear) → `menu` → `ui` (the page, then the debug
//! overlay). The last two load the target rather than clearing it, so declaring
//! the UI pass first would put the pause panel on top of the words it exists to
//! frame.
//!
