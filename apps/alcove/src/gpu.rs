//! Alcove's GPU side: the shared shell↔HAL join, the forward renderer over
//! [`crate::court`], and the UI and menu passes rule 4 asks every sample for.
//!
...
//! ring, resize and teardown. What is here is the part that is alcove's: a
...
//!
//! There is no second camera here — `apps/lantern` has one because its charter
//! asks for a render-to-texture monitor, and this fixture's charter asks instead
//! for **cost per technique, per frame**. [`OcclusionCost`] is that: the passes
//! whose labels the occlusion chain owns, read off [`PassTimers`], so a frame
//! drawn with `r_ssao_split` up reports the `ssao` and `ssao-shipped` gathers
//! separately and a frame drawn without it reports the one gather it ran.
...
/// The label prefix every pass in the occlusion chain carries.
///
/// `crcbl_render::ssao` records `ssao`, `ssao-shipped`, `ssao-blur`,
/// `ssao-blur-2` and `ssao-upsample`, and this is what picks them out of a
/// frame's whole report. A prefix rather than a list of five, because the list
/// is the render crate's and a chain that grows a pass would leave a copy here
/// naming the old set.
...
/// What the occlusion chain's own passes cost on the GPU, off the last frame
/// whose timestamps have landed.
///
/// **The charter's "cost per technique, per frame".** With `r_ssao_split` up the
/// frame records two gathers — `ssao` on the console's technique and
/// `ssao-shipped` on the one that ships — so the two rows here are what a person
/// compares; with the seam down there is one gather and one row, which is the
/// honest report of a frame that ran one technique.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct OcclusionCost {
    /// Each occlusion pass the frame ran, in execution order, and its cost in
    /// nanoseconds.
    pub passes: Vec<(String, u64)>,
    /// Whether this device has timestamp queries at all.
    ///
    /// Told apart from "the chain ran no passes", which is what a frame with
    /// `--no-ao` reports and is a different fact.
    pub timed: bool,
}
...
impl OcclusionCost {
    ...
    pub fn row(&self) -> String {
        ...
        if self.passes.is_empty() {
            return "no occlusion passes in this frame".to_string();
        }
        self.passes
            .iter()
            .map(|(label, nanos)| {
...
impl crcbl::ui::DebugModule for OcclusionCost {
    fn debug_section(&self, section: &mut crcbl::ui::DebugSection) {
        section.set_title("occlusion cost");
        if self.passes.is_empty() {
            section.row_str("chain", &self.row());
            return;
        }
...
impl Gpu {
    /// Opens the join and builds the forward renderer on [`court::court`].
    ///
    ...
    /// graphics queue or no surface format, if the court's description is one
    ...
    /// Builds the renderer, the court and the two UI passes on an already-open
    /// context — everything both bring-up paths share once the device exists.
    ///
    /// # Errors
    ///
    /// [`GpuError`] if the court's description is one the pools it asks for
    ...
    fn from_context(
        ...
        if report.is_empty() {
            crcbl::log::info!("alcove: device granted every optional feature asked for");
        } else {
        ...
        let placed = match court::place(&mut renderer) {
            ...
            Err(error) => {
                ...
                return Err(GpuError::Hal(crcbl::hal::HalError::InvalidDescriptor(
                    format!("alcove's court does not fit its own instance pool: {error}"),
                )));
            }
        };
        crcbl::log::info!("alcove: {placed} object(s) placed in the court");
    ...
    /// What the occlusion chain cost in the last frame whose timestamps landed.
    #[must_use]
    pub fn occlusion_cost(&self) -> OcclusionCost {
        let Some(timings) = self.timings() else {
            return OcclusionCost::default();
        };
        OcclusionCost {
            passes: timings
                .passes
                .iter()
                .filter(|pass| pass.label.starts_with(OCCLUSION_PASS_PREFIX))
    ...
    pub fn frame(&mut self) -> Result<FrameOutcome, GpuError> {
        ...
        // The court has one light and it does not move — see `court::sun` — so
        // the list handed over each frame is empty and the sun is the whole of
        // what lights it.
        self.renderer.set_lights(&[]);
        self.renderer
            .begin_frame(self.ctx.device(), &self.camera, &court::sun(), extent)?;
        self.menu
        ...
        if !self.dumped {
            crcbl::log::debug!("render graph for the alcove frame:\n{}", compiled.dump());
            self.dumped = true;
        }
...
/// There is no camera layer to write: this fixture has one view, so the stack a
/// view would ask for is the default one — `apps/lantern` is where a second view
...
impl Gpu {
    ...
    ///
    /// **This is how the AO-only view reaches the frame**: the `V` key and the
    /// pause panel's `AO VIEW` row both write `crcbl::debug_view`, the loop
    /// reads that variable once a frame, and this is the seam it puts the answer
    /// through — `ForwardRenderer::set_occlusion_view` under it.
    ///
    /// # Errors
    ///
...
/// to a device request and the trait's `request` does not, so this forwards what
...
/// `--force-geometry` and `--no-ao` therefore have no source there. A caller
...
mod tests {
    ...
    /// report rather than a number.
    ///
    /// A device with no timestamp queries, a frame that ran no occlusion pass —
    /// which is what `--no-ao` produces — and a frame that ran two gathers are
    /// three different facts, and a row that printed nothing for the first two
    /// would leave a reader unable to tell "the chain is free" from "nothing
    /// measured it".
    #[test]
    fn the_occlusion_cost_row_tells_untimed_from_unoccluded_from_compared() {
        let untimed = OcclusionCost::default();
        ...
        let unoccluded = OcclusionCost {
            passes: Vec::new(),
            timed: true,
        };
        assert!(
            unoccluded.row().contains("no occlusion passes"),
            "{}",
            unoccluded.row()
        );

        let compared = OcclusionCost {
            passes: vec![
                ("ssao".to_string(), 310_000),
                ("ssao-shipped".to_string(), 250_000),
            ],
            timed: true,
        };
        let row = compared.row();
        assert!(row.contains("ssao 0.310 ms"), "{row}");
        assert!(row.contains("ssao-shipped 0.250 ms"), "{row}");
        assert_ne!(
            untimed.row(),
            unoccluded.row(),
            "an untimed device and an unoccluded frame must not read alike",
        );
    }
}
