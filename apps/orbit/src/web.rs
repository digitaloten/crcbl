...
//!
//! `apps/orbit` is a `cdylib` on `wasm32-unknown-unknown`, and this module is
...
//!
//! What is left here is what is genuinely orbit's: the
...
//! accessor here, because orbit has nothing to read out of it: the flight keeps
//! no record between visits — a landing is a restart, not a save — and every
//! byte it draws with (the glyph atlas, the shaders) is compiled into the
...
//!
//! `__crcbl_orbit_` is this module's prefix. **What each of the ten lifecycle
...
//!
//! [`__crcbl_orbit_prepare`], [`__crcbl_orbit_log_level`],
//! [`__crcbl_orbit_boot`], [`__crcbl_orbit_frame`], [`__crcbl_orbit_status`],
//! [`__crcbl_orbit_shutdown`], [`__crcbl_orbit_error_ptr`],
//! [`__crcbl_orbit_error_len`], [`__crcbl_orbit_log_take`],
//! [`__crcbl_orbit_log_ptr`].
...
//
// The macro rather than a hand-written impl, which is what `apps/viewer` needs:
// it opens a file the command line named, so its `Options` has a field no
// default can fill in. Orbit's has none — every flag it takes is one of the
// shared set, and the browser wants all of them at their defaults — so
// `Options::default()` is the whole of what a page has to say.
//
...
crcbl::impl_web_pending!(PendingLoop, Loop, Options, crate::app::OrbitError);
...
crcbl::web_exports! {
    ...
    prepare: __crcbl_orbit_prepare,
    log_level: __crcbl_orbit_log_level,
    boot: __crcbl_orbit_boot,
    frame: __crcbl_orbit_frame,
    status: __crcbl_orbit_status,
    shutdown: __crcbl_orbit_shutdown,
    error_ptr: __crcbl_orbit_error_ptr,
    error_len: __crcbl_orbit_error_len,
    log_take: __crcbl_orbit_log_take,
    log_ptr: __crcbl_orbit_log_ptr,
}
