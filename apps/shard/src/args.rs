...
//! and this sample does not — which is where it will go.
...
/// What the command line asked shard for.
...
mod tests {
    ...
    /// The engine tests the shared flags; what is shard's to assert is that this
    /// sample's default tick rate reaches them, which the engine cannot know.
    #[test]
    ...
    /// The shared flags are documented in two places — here and in `crcbl::args`
    ...
    /// **Every key the sample binds is in `--help`.** A control nobody is told
    /// about is a control nobody presses, and the torch key is the one a visitor
    /// would never guess.
    #[test]
