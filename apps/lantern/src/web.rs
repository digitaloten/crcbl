...
//!
//! `apps/lantern` is a `cdylib` on `wasm32-unknown-unknown`, and this module is
...
//!
//! What is left here is what is genuinely lantern's: the [`WebPending`](crcbl::web::WebPending) impl,
...
//! accessor here, because lantern has nothing to read out of it:
...
//!
//! `__crcbl_lantern_` is this module's prefix. **What each of the ten
...
//!
//! [`__crcbl_lantern_prepare`], [`__crcbl_lantern_log_level`],
//! [`__crcbl_lantern_boot`], [`__crcbl_lantern_frame`],
//! [`__crcbl_lantern_status`], [`__crcbl_lantern_shutdown`],
//! [`__crcbl_lantern_error_ptr`], [`__crcbl_lantern_error_len`],
//! [`__crcbl_lantern_log_take`], [`__crcbl_lantern_log_ptr`].
...
// which stays here because the options it opens with are lantern's.
...
crcbl::impl_web_pending!(PendingLoop, Loop, Options, crate::app::LanternError);
...
crcbl::web_exports! {
    ...
    prepare: __crcbl_lantern_prepare,
    log_level: __crcbl_lantern_log_level,
    boot: __crcbl_lantern_boot,
    frame: __crcbl_lantern_frame,
    status: __crcbl_lantern_status,
    shutdown: __crcbl_lantern_shutdown,
    error_ptr: __crcbl_lantern_error_ptr,
    error_len: __crcbl_lantern_error_len,
    log_take: __crcbl_lantern_log_take,
    log_ptr: __crcbl_lantern_log_ptr,
}
