...
mod tests {
    ...
    /// the file instead of from memory — so every other assertion about the best
    /// would pass exactly as it does now. What is read is therefore that a
    /// second open starts over, which is the one reading a write would move.
    #[test]
