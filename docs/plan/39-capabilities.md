...
- **`off` is a rung, not a separate mechanism.** It is simply the lowest one, so
  ...
  meaning.
- **`antialiasing` is the one enumerated key that does _not_ clamp**, and it is
  an exception with a reason rather than a counter-example. Its rungs are one
  resolve slot rather than an amount of something, so `min` under any order
  answers "neither" where the camera asked for FXAA and the player asked for
  SMAA — which is not less of the camera's ask, it is nothing at all.
...
preset **command** below is what selects a column, and `custom` is what the
label reads when the covered keys are not any one column's set.

### The preset is a command, not a key, and it is not a fifth layer

`quality low` | `medium` | `high`, at the console. **Built 2026-08-31** —
...
**It is a command rather than a catalogue key, and that is a correction to this
section (2026-08-31).** A key has to hold a value, and the only value this one
could hold is a label describing what was written into the other keys. This
section used to argue that such a label "cannot drift, because nothing reads it
back" — but the console prints every key, so something does read it back, and a
file holding `quality = "high"` beside a hand-edited `render_scale` would read
back as `high` and be wrong. It would also have to answer what
`quality = "custom"` _writes_, which is nothing. So the label is **derived**
instead: `presets::selected` compares what the engine's own readers answer
against each column and names the one that matches. `custom` is then exactly
"the covered keys are not any one column's set" — a hand-edited file, a row the
player moved, and a run that has selected nothing, all of which are the same
fact. Touching an individual key still takes the label off its tier; nothing has
to remember to invalidate it.

`ultra` is dropped from the domain: the tier table has three columns, and a
fourth name with no column behind it is a word that cannot be given values.
...
The medium and high AA cells say **CMAA2**, which is not built:
[49-antialiasing.md](49-antialiasing.md)'s eighth decision puts CMAA2 and SMAA
1x in one tier and retires SMAA in the slice that lands CMAA2, so the rung above
FXAA is `Antialiasing::Smaa` until that day and `QualityPreset::values` moves
with it.

### The catalogue

Rungs are written lowest-first, which is the clamp order for that key. **The
"Today" column is per row on purpose**; of the whole table, the effect rows are
implemented as booleans and most of the rest have no reader and no renderer
half.
...
| `anisotropic_filtering` | `1` \| `2` \| `4` \| `8` \| `16`       | **Built 2026-08-29** as a value key on `render_scale`'s pattern: `crcbl::settings::anisotropic_filtering` reads it, `1` through the desktop preset's sixteen, and `ForwardRenderer::set_anisotropy` clamps it to the device's own `Limits::max_sampler_anisotropy`. `apps/options`'s `ANISOTROPY` row steps its five values.                                                                                                                                                                   |
