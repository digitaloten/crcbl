...
"editor-authored": `apps/editor` does not exist, so the committed directory is
generated by `Scene::save` from the constants in `map.rs` and held to them byte
for byte by `the_committed_map_is_what_the_writer_writes`.
