...
module header of its `src/lib.rs`. **arena and mirrors have no `apps/` directory
at all**, and each doc says what it is waiting on — for 17 that is the ladder
itself, since a comparison fixture holding one technique is not a comparison.
**alcove (19) is the first of the comparison wave to ship**: `apps/alcove` was
built natively on 2026-09-04, once the occlusion chain had a second technique to
compare, and its browser demo landed the same day at `/demos/alcove/` — the
first page on the site whose controls are HTML rather than keys, because a seam
walked with `,` and `.` is a comparison a phone cannot reach. **sundial (18)
followed it the same day**, once the shadow ladder had three filters and a seam
to compare them across, and its page landed with it at `/demos/sundial/` — the
second on the site whose controls are HTML, and the only one that puts a
simulation's own clock among them, because a sun nobody can stop is a fixture
whose artefacts cannot be looked at. options (20) shipped on 2026-08-28.
