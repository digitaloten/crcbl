...
below. **Rung 3 landed whole on 2026-09-06**: the packed
metallic-roughness-occlusion page and the emissive page are bound and
`mesh.slang` shades through them, and the importer reads
`metallicRoughnessTexture`, `occlusionTexture` and `emissiveTexture` and fills
all four page columns of the row — so a shelf model's whole metallic-roughness
set draws. What is still missing from the list above is
`occlusionTexture.strength`, which no row has a field for; `docs/backlog.md`
holds it. Each rung's own "landed" paragraph says exactly what shipped.
...
4. **Alpha modes. `MASK` landed 2026-09-05**, and §3 below is where what it cost
   ...
   both on import, and `mesh.slang`'s `alpha_masked` is the one comparison every
   stage that draws the surface makes — the shaded one, the depth-only one
   behind the prepass and the shadow atlas, and the reflective shadow map.
   `BLEND` is §3's second step and is not built: the importer records it as
   `OPAQUE` and warns with the count.

   **`doubleSided` landed the same day**, and it is the second material _mode_
   rather than a third alpha mode. `GpuMaterial::DOUBLE_SIDED` is the second bit
   of `MODE_MASK`, which fills the two bits `GpuInstance::MATERIAL_MODE_MASK`
   had reserved, and glTF 2.0 §3.9.6 is honoured in both halves: the renderer
   builds a `CullMode::None` twin of every pipeline that draws a surface — the
   colour pass's, the depth-only one, the cutout one, the reflective shadow
   map's and the wireframe view's — and `mesh.slang`'s `double_sided_normal`
   reverses the shading normal on a back face before the lighting equation is
   evaluated, in the shaded stage and in the reflective shadow map's. The
   importer sets the bit; it dropped the flag silently before, without even
   counting it. A leaf card authored as a cutout is nearly always double-sided,
   so this is the half of rung 4 that makes foliage look right, and a material
   can carry both modes at once.

   **The mesh path needed a third piece.** A cluster's normal cone means "draws
   nothing" only under the back-face cull, which is exactly what `doubleSided`
   switches off — so `mesh_cluster.slang`'s `cone_may_reject` and
   `crcbl_render::cull::cone_may_reject` skip the cone rejection for an instance
   whose material mode carries the bit, and leave the frustum half and the
   radius term alone. Without it a double-sided open surface loses its
   back-facing clusters on the mesh-shader path and draws on every other one,
   which is what `Scene::DoubleSided`'s two-path comparison asserts against.
...
**The page allocator, generalised — LANDED 2026-09-06 (the foundations block's
fourth row).** `crcbl_render::scene::PageDesc` is a table over a new `PageKind`
— `BaseColor` and `Normal` today, and the metallic-roughness-occlusion and
emissive pages of this section are one variant each — with its own extent and
its own layer list per kind: `empty`, `set_extent(kind, extent)`,
`push_layer(kind, texels)`, `extent(kind)` and `layers(kind)`. Four page indices
in the material row is what the 64-byte `GpuMaterial` above is sized for, and
`GpuMaterial::NO_PAGE` already means the same out-of-band thing on all four
columns, so a new kind adds no convention.

It landed in two steps. The first took `NO_PAGE` out of band — `0xFFFF`, one
past `MAX_PAGE_LAYER` — so `mesh.slang`'s new `base_color_texel` returns the
literal `float4(1, 1, 1, 1)` for a row that names no page instead of sampling a
white layer, and `shading_normal_of` tests the sentinel where it tested zero.
That is the step that could have moved a picture, and
`crcbl/tests/mesh_e2e/base_color_page.rs`'s
`a_row_naming_no_page_shades_exactly_as_a_white_layer_does` is the device
evidence that it did not: one quad drawn twice, once through a row naming no
page and once through a row naming an all-white layer, read back bit for bit
equal on radv and on lavapipe. The second generalised the type and shifted every
producer's layer constants down by one, which by construction cannot move a
frame — a mistake in it is a refusal from `with_scene` naming the kind, or a
golden.

**What it bought.** A kind nothing names holds no layers, and the renderer
creates a 1×1 placeholder image for the binding rather than an `extent²` one:
every scene in this tree without a normal map used to carry a full neutral layer
and a third again for its chain, and `apps/alcove`, `apps/sundial` and
`apps/quarry` — which name no page at all — now allocate two texels between
them. The importer sizes each kind from its own images, so a document with a
2048² albedo beside a 512² normal map stops resampling the normal map up onto
...
default of one; the importer does not read it either, and `docs/backlog.md`
holds what a row would need to carry it.
...
**The importer half, LANDED 2026-09-06 as well.** `crcbl_scene::gltf_import`
reads `pbrMetallicRoughness.metallicRoughnessTexture`, `occlusionTexture` and
`emissiveTexture` and reports each on the seam `baseColorTexture` and
`normalTexture` already rode — `GltfScene::metallic_roughness_textures`,
`occlusion_textures` and `emissive_textures`, each saying which image and which
UV set, with the page layer left to whoever builds the page.
`crcbl_scene::gltf_render::pack_page` packs all four kinds and returns a
`[Vec<Option<u32>>; PageKind::ALL.len()]` table on `PageKind::index`'s terms,
one entry per material per kind, which is what lets the packed page — built from
a _pair_ of images rather than one — be answered for at all.

**The packing is glTF's own split, both ways round.** `metallicRoughnessTexture`
supplies `g` and `b` and `occlusionTexture` supplies `r`. Where the two name the
**same image** the layer is that image as it stands, which is the convention
BoomBox, WaterBottle, Corset, BarramundiFish and FlightHelmet all ship; where
they name **different** ones — SciFiHelmet — the occlusion image's `r` is
resampled onto the page's extent and written into the packed layer. A channel
with no image behind it is `0xFF`, the identity of each of the three products,
so a material naming only one of the two slots keeps its factors across the
rest. A layer is keyed by the pair, so two materials naming one pair share it.
`crcbl/tests/gltf_e2e.rs`'s
`an_imported_emissive_texture_lights_the_half_of_the_quad_it_covers` is the
device evidence that a `.glb`'s own emissive image reaches a frame; the channel
arithmetic is `mesh_e2e`'s, above.
...
**The importer half landed 2026-09-06**
(`3a44b5c feat(scene): import the packed and emissive maps`):
`crcbl_scene::gltf_import` reads all five maps — `base_color_texture`,
`normal_texture`, `metallic_roughness_texture`, `occlusion_texture` and
`emissive_texture` — so a document in this tree reaches every page, and the
delivery table's two rows below are LANDED rather than open.
...
page's shape — `layers × extent² × 4` bytes and a third again for the chain, so
21 MiB a layer at 2048² — at an extent of its own since row (d) landed on
2026-09-06, and with no layer burned on a neutral texel: a scene that names no
normal map pays one texel rather than one layer. **The browser tier pays the
same arithmetic and more bytes**: WebGPU has `dpdx`/`dpdy` and a seventeenth
sampled-texture binding is inside every portability floor this engine holds to,
but the page is uncompressed `RGBA8` until the KTX2/BC5 rung lands, where a
desktop would spend one byte a texel instead of four — which is why the
compression rung is the one the material ladder waits on. Estimated; no browser
measurement was taken. **The ray-tracing tier cannot use half of this**: a hit
shader has no quad and therefore no derivatives, so a traced hit can only be
shaded through an authored tangent frame, and an unmarked mesh reached by a ray
has no normal mapping at all. That is an argument for MikkTSpace ahead of §5's
rung, and it is in the backlog with the rest.

**What the cutout cost, measured 2026-09-05.**
...
on an RX 7900 XTX under radv and on lavapipe. Lantern masks nothing, so it is
the control: at 9bef4d5 and with this rung landed it draws through the same
vertex-only pipeline, and the two builds agree to within their own run-to-run
spread — `shadow` **0.142 → 0.140 ms** on radv (spreads 0.141–0.144 and
0.139–0.140) and **9.481 → 9.511 ms** on lavapipe, `depth-prepass` **0.038 →
...
**What the double-sided mode costs is the same bucket table and nothing else,
measured 2026-09-05.** Same command, same machine, medians of three runs a
configuration. Lantern as shipped is all single-sided and is the control:
`shadow` **0.137 ms** on radv and **9.321 ms** on lavapipe, `depth-prepass`
**0.038** and **1.386**, `forward` **0.341** and **32.705** — every one of them
inside the spread of the per-bucket control above. With `room::MONITOR` marked
`DOUBLE_SIDED` by an uncommitted edit — one row of six, worn by one instance of
the twelve `room::OBJECTS` places — `shadow` reads **0.221** and **9.997**,
`depth-prepass` **0.042** and **1.442**, `forward` **0.319** and **32.746**.

Those are the masked build's own numbers to three figures on radv, which is what
identifies the cost: **0.221 ms of shadow is the twelve extra indirect
dispatches per shadow view**, exactly as the fourth configuration above showed
for an unmasked lantern with an unconditional empty twin, and the forward pass
moves _down_ by the same amount in both. A `CullMode::None` pipeline rasterises
the same triangles the culling one does and one more side of each; on this scene
that is not measurable beside the dispatch count. On lavapipe the twin costs
about seven per cent of the atlas, against the raster tier's sixty.
...
Apple or Windows hardware. The same is true of the cull-none twins: a tiler's
early-depth and hidden-surface removal are what a two-sided surface costs most,
and nothing here can say what.
...
3. **`texture_quality` as a `lod_min` clamp**, which is all the cheap form of
   ...
   rung.
4. **Block compression: KTX2 carrying Basis Universal, decided 2026-09-06.** The
   device half is done — `Format::Bc7RgbaUnormSrgb` for colour, `Bc5RgUnorm` for
   the two-channel normal [44-lighting.md](44-lighting.md)'s rung 2 asks for,
   `Bc4RUnorm` for a mask or a lone roughness — and what was open was the
   encoder. It is chosen, and both halves are new dependencies the user has
   taken:
   - **The container is KTX2 and the payload is UASTC, supercompressed.** That
     is Khronos' own pipeline, it is what glTF's `KHR_texture_basisu` names, and
     it is what three.js, Babylon and Bevy ship for the web tier. One asset
     transcodes at load rather than one asset per device family, which is the
     property that matters here: the browser is a first-class target and BC is
     an optional WebGPU feature, so a BC-only pipeline would ship uncompressed
     bytes to part of the site. UASTC rather than ETC1S because these are
     material pages — a normal map through ETC1S is the artefact that pipeline
     is known for.
   - **The transcode target is the device's answer**, resolved at load: BC7 (and
     BC5/BC4 for the two- and one-channel pages) where
     `Features::TEXTURE_COMPRESSION_BC` was granted, ASTC or ETC2 where it was
     not. `TEXTURE_COMPRESSION_BC` is the only one of the three this seam has a
     bit for today; ETC2 and ASTC each need their own `Features` bit and their
     own `Format` variants, and adding them is the seam change this rung carries
     on the day a device without BC is targeted — a capability question in
     [39-capabilities.md](39-capabilities.md)'s sense, answered through
     `Features`, not a renderer branch.
   - **The encoder is the `basisu` CLI, pinned, driven by the bake tool** —
     exactly the arrangement `crates/crcbl-shaders/tools/compile-shaders.sh`
     already has for `slangc`: the version lives in one place, the tool runs it
     by path, and a `--check` mode is what proves a committed artifact still
     matches its source. A CLI rather than a linked encoder because encoding is
     an offline step, and because the pin is what makes the output reproducible;
     two encoder releases legitimately produce different blocks from identical
     input.
   - **The loader is the `basis-universal` crate with its transcoder feature,
     plus `ktx2` for the container.** Transcoder only: the engine never encodes
     at run time, so the encoder half of that crate is not linked into a game or
     into a wasm build.
   - **The importer reads `KHR_texture_basisu`.** The `gltf` crate this
     workspace resolves exposes no feature for that extension, so
     `crcbl_scene::gltf_import` reads it out of the document's JSON by hand —
     the extension names a `source` on the texture the way the base document
     does, so it is one more branch in the same place, not a second import path.

   **It is the bandwidth rung**: four to six times less texture traffic, which
   is what makes normal maps and the material pages affordable in the browser at
   all. Until it lands a page is uncompressed and four times its BC7 size — a
   memory figure rather than a picture, which is why the rung is last in this
   section and near the top of the delivery table.

## 3. Transparency — absent, structurally

> **The rung moved to [53-transparency.md](53-transparency.md) on 2026-09-06**,
> where the five decisions the blended pass turned on, the sort key's layout,
> what each crate gains and the fixture that holds it are written out. This
> section keeps the ordering rule and nothing else.
...
**Where this one is:** alpha-mask landed 2026-09-05 — a `discard`, no sorting,
no new pass, and what foliage actually wants. The blended pass with GPU-sorted
keys is next and is `53`'s.

**Why order-independence is decided before it is reached, and not after:**
weighted-blended OIT is the cheap candidate, and it is an _approximation_ that
cannot be blessed against a reference — which this workspace's golden discipline
has to settle before such a pass is built, because a golden blessed against an
approximation records the approximation as the answer. So the order is sorting
first, and an order-independent scheme only if the sorting proves insufficient;
`53`'s first decision is that refusal, taken 2026-09-06.
...
scene. It landed first, 2026-08-27, for the same reason FXAA landed before CMAA2
...
| Colour grading | 3D LUT              | **missing**, specified 2026-09-06 in [48](48-post-processing.md) |
...
**Where this one is:** at the standard. A gradient and a physically based
atmosphere, either of which lights the scene, is what a missed reflection falls
back to, and is drawn behind the frame.
...
### The atmosphere — Hillaire's, and it is built (2026-09-05)

"A Scalable and Production Ready Sky and Atmosphere Rendering Technique"
(Hillaire, EGSR 2020) over Bruneton and Neyret's Earth.
[`crcbl_shaders::atmosphere`](../../crates/crcbl-shaders/src/atmosphere.rs) is
the module and every coefficient in it is a named constant with the paper it
comes from.

**Two of its three LUTs are cooked and committed**, in
`crates/crcbl-shaders/tables/atmosphere.bin`: the transmittance LUT and the
multiple-scattering LUT depend on the planet and not on the sun, so neither is
ever rebuilt at run time. `cook-atmosphere` regenerates them —
[`crates/crcbl-shaders/tools/cook-atmosphere.rs`](../../crates/crcbl-shaders/tools/cook-atmosphere.rs)
— and `cook-atmosphere --check` holds them to their integrators in CI beside
`cook-dfg`'s. One artifact for the two, because the multiple-scattering
integrator reads the transmittance table it was just handed and a tree with one
regenerated and the other not would have no meaning.

**The third is marched on the host.** `SkyView::build` walks every texel of a
small sun-dependent LUT out of multiplies, divisions, square roots and
`fog::exp_neg` — no `sin`, no `pow`, no `exp` from a platform's `libm` — so §4's
rule holds and the same bytes reach every backend. `sky.slang` reads it as a
...
azimuth _angle_, both of which need a trigonometric function to build or to
read. This one indexes the same field by the direction's `y` through
`sign(s)·s²` and by the **cosine** of the azimuth away from the sun through
`1 − 2u²`: both maps and both inverses are multiplies and square roots, both
crowd texels where the field changes fastest — the horizon, and the sun's own
limb — and the field being stored is still the paper's.

**What it costs.** Priced 2026-09-05 on
`apps/lantern --headless --frames 400 --size 1920x1080` with a temporary
atmosphere in place of no sky at all, off `crcbl_render::PassStats`, medians of
three runs each:

| 1920×1080                  | radv (RX 7900 XTX, Mesa 26.2.2) | lavapipe (same Mesa) |
| -------------------------- | ------------------------------- | -------------------- |
| `sky` pass, p50 / p95      | 0.004 / 0.005 ms                | 0.419 / 0.482 ms     |
| whole frame, p50, with sky | 1.469 ms                        | 92.45 ms             |
| whole frame, p50, no sky   | 1.471 ms                        | 92.45 ms             |

The pass is under a third of a per cent of the frame on the desktop tier and
...
**What is left above this** is a cubemap sky, for a scene whose environment is
an authored image rather than a planet. It is its own rung and is not blocked by
...
| Rung                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Why here                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **THE LIGHTING ORDER, decided 2026-08-30** — the raster items interleave with the foundations rather than waiting behind them. Every rung it enumerated has landed, and the rows below are the record of which and when. What it still binds is two placements and one rule: **(g) lands where the first rung that needs it does; (c) when the RT tier's updater is next**, and the user's rule for the order — best-looking for the performance, cheapest real win first |
...
| **(b) The render stack as RON, per camera — LANDED 2026-09-06**                                                                                                                                                                                                                                                                                                                                                                                                           | [48-post-processing.md](48-post-processing.md) — the pass list a camera runs is data, so a demo, a test and a settings preset compose it without a build. `crcbl_render::stack::CameraStack` is the file: one optional pass per `RenderEffects` bit, `compile` is a bit per `Some`, `from_ron` refuses an unknown field by line and column and `to_ron` writes it back deterministically. The `ron` crate arrived with it, which is the first thing in this workspace to read or write RON. `ForwardRenderer::set_camera_stack` writes the camera layer alone; `apps/lantern/assets/camera.ron` is the file a demo draws through and `--stack` reads another at run time; `crcbl::screenshot`'s bloom fixture composes one from a string literal. **Pass parameters arrive with each pass's own setter** — `set_fog`, `set_exposure_adaptation`, `set_tonemap_curve` — so every pass type but the antialiasing slot is field-less today, and `docs/backlog.md` lists which are waiting for a serialized form. The two antialiasing bits are one field naming a tier, on [49-antialiasing.md](49-antialiasing.md)'s terms |
| **(c) Acceleration structures and inline ray queries on the seam** (vk, dx12, mtl)                                                                                                                                                                                                                                                                                                                                                                                        | §5 — the GI's engine on the ray-tracing tier: a build and refit, a `Capability` the device reports, and the hit-shading reads. Decided 2026-08-30 in `docs/backlog.md`: GI is hardware ray tracing only; the other tiers run the raster stack. **The seam's shape is outlined below the table**, because four documents defer to this row and none of them names anything more than a `Capability`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **(d) The page allocator, generalised — LANDED 2026-09-06**                                                                                                                                                                                                                                                                                                                                                                                                               | §2 — `PageDesc` is a table over `PageKind`, one extent and one layer list each, and a kind nothing names costs one placeholder texel. Rung 3's two pages are one variant apiece. The shadow atlas is not a caller: it got its own quadtree on 2026-08-31, and §2's paragraph says why. Decal atlases are not built                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **The viewer as the PBR showcase — LANDED 2026-09-06** — native drag-and-drop, a bundled shelf of Khronos CC0 models picked from a list, Suzanne on open ([sample/05-viewer.md](sample/05-viewer.md) milestone 4)                                                                                                                                                                                                                                                         | **The shelf and Suzanne-on-open shipped 2026-08-30** (`apps/viewer/src/shelf.rs`, nine models, Suzanne the default); drag-and-drop works on both hosts bar X11, which `sample/05-viewer.md` records. The full material set followed on 2026-09-06 with rung 3's two halves: `mesh.slang` samples `mro_textures` and `emissive_textures`, and the importer fills the two columns that point at them, so all four maps a document ships now draw. What is left is `occlusionTexture.strength`, which `docs/backlog.md` carries                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **Emissive page — LANDED 2026-09-06** (the factor shipped 2026-08-27)                                                                                                                                                                                                                                                                                                                                                                                                     | §2's rung 3, both halves: `PageKind::Emissive` in `Rgba8UnormSrgb`, `mesh.slang`'s `emissive_texel`, and the importer's `emissiveTexture`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Alpha-mask and double-sided materials — landed 2026-09-05**                                                                                                                                                                                                                                                                                                                                                                                                             | §3's first step and §2's rung 4 both, built: `GpuMaterial::ALPHA_MODE_MASK` beside the cutoff, `mesh.slang`'s `alpha_masked` in the shaded stage, in the new `depthMaskedFragmentMain` the depth prepass and the shadow atlas take when the material table masks anything, and in the reflective shadow map. No sorting and no new pass. `GpuMaterial::DOUBLE_SIDED` is the second mode bit beside it: a `CullMode::None` twin of every pipeline that draws a surface, and `double_sided_normal` reversing the shading normal on a back face — glTF 2.0 §3.9.6's two halves. Both modes route per draw bucket, and a scene emits a twin per mode its own materials carry                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Blended transparency with GPU-sorted keys**                                                                                                                                                                                                                                                                                                                                                                                                                             | [53-transparency.md](53-transparency.md) — the first rung here that touches the frame's structure, and the raster ladder's last. Its five decisions were taken 2026-09-06: sorted alpha blending rather than OIT, one indirect call per blended slot on every `EmitTail`, no shadows from blended surfaces, the pass after `ssr` and `volumetric-composite` with the fragment applying the froxel fog itself, and BLEND exclusive with MASK for six material modes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Specular antialiasing (roughness regularisation) — landed 2026-09-05**                                                                                                                                                                                                                                                                                                                                                                                                  | Tokuyoshi and Kaplanyan's isotropic kernel (I3D 2019) in `mesh.slang`'s `specular_aa_kernel`: the screen-space derivatives of the shading normal give the variance the pixel could not resolve, and twice that — clamped to the paper's `SPECULAR_AA_KAPPA` — widens GGX's `alpha2` before the lobe. The direct lobe alone; the `dfg` and `ltc` taps and the reflectivity attachment keep the material's own roughness. Exactly zero on a facet whose normal does not vary, so the flat-normalled goldens are unchanged to the bit. The one golden with real curvature, the dunes cluster DAG, is regularised and re-blessed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Block-compressed pages: KTX2 and Basis at the bake, BC7/BC5/BC4 on the device**                                                                                                                                                                                                                                                                                                                                                                                         | §2's filtering subsection, which now carries the specification — **the encoder was chosen 2026-09-06**: a pinned `basisu` CLI driven by the bake tool the way `compile-shaders.sh` pins `slangc`, KTX2 carrying supercompressed UASTC, and the `basis-universal` (transcoder feature) and `ktx2` crates at load. **It is the bandwidth rung** (2026-08-30): four to six times less texture traffic, which is what makes normal maps and the material pages affordable in the browser at all                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Colour grading, DOF, lens artefacts                                                                                                                                                                                                                                                                                                                                                                                                                                       | [48-post-processing.md](48-post-processing.md)'s grading section, which specifies the first of the three — a post-tonemap 32³ `Rgba8Unorm` LUT authored as `.cube`, identity when absent, a `CameraStack` field, applied inside the tonemap pass so the resolve reads a graded luma (decided 2026-09-06). DOF and lens artefacts follow it: both want the serialized-parameter shape the LUT field settles, and neither has a reason to precede a curve and a grade                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **~~CMAA2 in SMAA's place (landed 2026-09-06)~~, then MSAA 2×/4×/8×**                                                                                                                                                                                                                                                                                                                                                                                                     | [49-antialiasing.md](49-antialiasing.md)'s eighth decision — the settings row landed 2026-08-30 and CMAA2 with SMAA's retirement 2026-09-06; what remains is the MSAA rungs and the depth resolve they need                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **The probe volume moves — scrolling landed 2026-09-05** — the visibility maps and the clipmap landed 2026-09-02, the sun's RSM updater 2026-09-04 with lantern's and shard's bakes gone with it, the punctual producer the same day, and the clipmap's whole-step scroll with recapture the day after                                                                                                                                                                    | [50-irradiance-probes.md](50-irradiance-probes.md)'s decision of 2026-08-30 — leak-free, one bounce, dynamic sun and lamps on every tier. Both producers are built, so lantern's coloured wall tints the plaster beside it again, and `ForwardRenderer::follow_probe_volume` re-centres every level on a tracked point by whole steps with `probe_capture::recapture` taking the exposed rows again, so a probe the camera leaves behind is captured where it lands. What the scroll still owes — the step's device drain, the recapture's transients and a caller that scrolls in anger — is `docs/backlog.md`'s. The traced updater rides on (c)                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Temporal SSR, TAA, temporal upscaling                                                                                                                                                                                                                                                                                                                                                                                                                                     | §9's pass is built; each is its own rung now. **SSGI is not among them** — it was withdrawn from this ordering on 2026-08-30, because [50-irradiance-probes.md](50-irradiance-probes.md)'s probe volume is a leak-free bounce on every tier where SSGI approximates the same thing in screen space on one; §5 records the withdrawal                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |

### Foundation (c): the acceleration-structure seam, in outline

Four documents defer to row (c) above — `ROADMAP.md`'s P7C,
[sample/18-sundial.md](sample/18-sundial.md)'s milestone 5,
[sample/19-alcove.md](sample/19-alcove.md)'s ray-traced rung, and lantern's
both-paths comparison at S4B — and until 2026-09-06 the only thing any of them
could point at was a capability name. This is what they point at instead. It is
an outline of the seam's shape, not a design of the passes above it: the passes
are P7C's, and P7C is scheduled after [53-transparency.md](53-transparency.md),
the raster ladder's last rung.

**The three APIs agree on more than they disagree on**, which is what makes one
seam possible at all — `VK_KHR_acceleration_structure` with `VK_KHR_ray_query`,
DXR 1.1's inline ray tracing, and Metal's `intersection_query`. Each has a
two-level structure built from geometry on the device, each refits that
structure in place when only the transforms moved, and each answers a query
issued from an ordinary shader stage with no separate hit shaders and no shader
binding table. So the seam takes the intersection of the three:

- **An acceleration-structure resource**, on `crcbl-hal`'s existing resource
  terms — a handle, a description, and a destruction that follows the frame's
  retirement like every other. Two levels, because all three APIs have exactly
  two.
- **Two encoder verbs: build and refit.** Build takes the geometry ranges; refit
  takes an already-built structure and the transforms that moved. Refit is the
  separate verb rather than a flag on build because that is the distinction the
  frame budget turns on — a rebuild is per-scene-change work and a refit is
  per-frame work, and a seam that spelled them the same would hide which one a
  caller was paying for.
- **`Capability::RayQuery`.** One capability, answered by a `match` like every
  other in [39-capabilities.md](39-capabilities.md)'s enum. `crcbl-webgpu`
  answers it with an `ApiAbsence` divergence — WebGPU has no ray tracing at any
  size — which is the same shape as its `BindlessDescriptorArray` answer and is
  permanent rather than a gap.
- **Inline ray queries from compute**, and only from compute to begin with. All
  three APIs allow a query from any stage; restricting the seam to compute is
  what keeps it to one shader shape rather than three, and the consumer the
  updater needs is a compute pass anyway.

**What the seam deliberately does not carry**: ray-tracing pipelines, hit
groups, a shader binding table, or any of the callable-shader machinery. Those
are the parts the three APIs spell differently, and inline queries are what
every one of the four dependents above actually needs — a shadow ray, an
occlusion ray, a reflection ray. Adding the pipeline form later is additive; the
inline form is not something it subsumes.
